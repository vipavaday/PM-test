...
import { MDBTvShowJSON, MDBMovieJSON, MDBResultJSON, MDBCreditsJSON, MDBCastJSON } from './content-parser.service.interface';

@Injectable({
  providedIn: 'root'
})
export class ContentParserService {
  public parse(json: MDBTvShowJSON | MDBMovieJSON, credits: MDBCreditsJSON, imgBaseUrl: string, content?: Content): Content {
  ...
  public parseTvShow(json: MDBTvShowJSON, credits: MDBCreditsJSON, imgBaseUrl: string, content = new Content()): Content {
    ...
    content.directors = json.created_by.map(creator => creator.name);
  ...
  public parseMovie(json: MDBMovieJSON, credits: MDBCreditsJSON, imgBaseUrl: string, content = new Content()): Content {
    ...
    content.originCountries = json.production_countries.map(country => country.iso_3166_1);
  ...
  private parseResult(json: MDBResultJSON, credits: MDBCreditsJSON, imgBaseUrl: string, content: Content): Content {
    ...
    content.genres = json.genres.map(genre => genre.name);
  ...
  private getCasts(credits: MDBCreditsJSON, imgBaseUrl: string): Cast[] {
    return credits.cast.map(json => this.parseCast(json, imgBaseUrl));
  }
